...
import org.giavacms.common.model.LocalLazyDataModel;
 ...
 * @param <T>
 */
public abstract class AbstractLazyController<T> implements Serializable
{
   ...
   public void updateInline()
   {
      ...
      if (getRepository().update(rowElement))
      {
         ...
         {
            if (getId(rows.get(rowIndex)).equals(getId(rowElement)))
            {
               break;
            }
         }
